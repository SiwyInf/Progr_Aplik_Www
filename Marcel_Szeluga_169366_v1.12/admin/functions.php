...
function addToCart($productId, $productName, $priceNetto, $quantity) {
    global $link; // Upewniamy się, że połączenie z bazą danych jest dostępne
    ...
    $query = "SELECT ilosc FROM products WHERE id = ?";
    if ($stmt = $link->prepare($query)) {
        ...
        // Sprawdzenie, czy wystarczająca ilość jest dostępna
        if ($availableQuantity < $quantity) {
            echo "<p>Brak wystarczającej ilości produktu w magazynie.</p>";
            return;
        }
        $stmt->close();
    }
